...
In-Depth Supply Chain Contagion Engine
Extracts SEC 10-K Risk Factors and categorizes Geopolitics, Routes, and Governance risks using local Ollama.
"""
import os
import json
...
def fetch_sec_risk_factors(ticker: str) -> str:
    ...
    if not api_key:
        print("SEC_API_KEY missing. Returning mock risk vector for demonstration.")
    ...
    try:
        ...
        # Truncate to reasonable length for local LLMs (~10000 chars)
        return item_1a_text[:10000]
    except Exception as e:
        print(f"SEC API Error: {e}")
        return mock_sec_risk(ticker)

def categorize_risks_with_ollama(risk_text: str, ticker: str) -> dict:
    """Pass unstructured SEC risk text to Ollama and output categorized JSON."""
    ...
    try:
        ...
        return {
            ...
            "summary": data.get("summary", "Summarized by Ollama.")
        }
    except Exception as e:
        print(f"Ollama Risk Extraction Error: {e}")
        return {"geopolitics": 3, "governance": 3, "supply_routes": 3, "summary": "Failed to parse risk data via local LLM."}
...
def analyze_supply_chain_contagion(ticker: str, peer_sentiment: float) -> dict:
    """Main Orchestrator for Phase 1e."""
    raw_sec_text = fetch_sec_risk_factors(ticker)
    structured_risks = categorize_risks_with_ollama(raw_sec_text, ticker)
